    ...
    def get(self, user_id: str) -> Iterable[Union[Mapping, int, None]]:
        ...
        try:
            ...
            if resources and table_key in resources and len(resources[table_key]) > 0:
                result[table_key] = PopularTableSchema(many=True).dump(resources[table_key])
            ...
            if resources and dashboard_key in resources and len(resources[dashboard_key]) > 0:
                result[dashboard_key] = DashboardSummarySchema(many=True).dump(resources[dashboard_key])
    ...
    def get(self, user_id: str) -> Iterable[Union[Mapping, int, None]]:
        ...
        try:
            ...
            if len(resources['table']) > 0:
                return {'table': PopularTableSchema(many=True).dump(resources['table'])}, HTTPStatus.OK
